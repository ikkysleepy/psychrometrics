...
class Psychrometrics {
  constructor() {
    ...
    var MIN_HUM_RATIO = -0.02; // Minimum acceptable humidity ratio used/returned by any functions.
    ...
    this.GetTDewPointFromVapPres = function (
      ...
      if (
        VapPres < this.GetSatVapPres(BOUNDS[0]) ||
        VapPres > this.GetSatVapPres(BOUNDS[1])
      )
        throw new Error(
    ...
    this.GetTWetBulbFromHumRatio = function (
      ...
      TDewPoint = this.GetTDewPointFromHumRatio(
        TDryBulb,
        BoundedHumRatio,
        Pressure
      );
    ...
    this.GetHumRatioFromTWetBulb = function (
      ...
      if (this.isIP()) {
        if (TWetBulb >= FREEZING_POINT_WATER_IP)
          HumRatio =
            ((1093 - 0.556 * TWetBulb) * Wsstar -
              0.24 * (TDryBulb - TWetBulb)) /
        ...
        if (TWetBulb >= FREEZING_POINT_WATER_SI)
          HumRatio =
            ((2501 - 2.326 * TWetBulb) * Wsstar -
              1.006 * (TDryBulb - TWetBulb)) /
            (2501 + 1.86 * TDryBulb - 4.186 * TWetBulb);
        else
          HumRatio =
            ((2830 - 0.24 * TWetBulb) * Wsstar -
              1.006 * (TDryBulb - TWetBulb)) /
    ...
    this.GetDryAirDensity = function (
      ...
      if (this.isIP())
        return (
          (144 * Pressure) / R_DA_IP / this.GetTRankineFromTFahrenheit(TDryBulb)
        );
    ...
    this.GetDryAirVolume = function (
      ...
      if (this.isIP())
        return (
          (R_DA_IP * this.GetTRankineFromTFahrenheit(TDryBulb)) /
          (144 * Pressure)
        );
    ...
    this.GetTDryBulbFromEnthalpyAndHumRatio = function (
      ...
      if (this.isIP())
        return (
          (MoistAirEnthalpy - 1061.0 * BoundedHumRatio) /
          (0.24 + 0.444 * BoundedHumRatio)
        );
      else
        return (
          (MoistAirEnthalpy / 1000.0 - 2501.0 * BoundedHumRatio) /
    ...
    this.GetHumRatioFromEnthalpyAndTDryBulb = function (
      ...
      if (this.isIP())
        HumRatio =
          (MoistAirEnthalpy - 0.24 * TDryBulb) / (1061.0 + 0.444 * TDryBulb);
      else
        HumRatio =
          (MoistAirEnthalpy / 1000.0 - 1.006 * TDryBulb) /
          (2501.0 + 1.86 * TDryBulb);
    ...
    this.GetSatAirEnthalpy = function (
      ...
      return this.GetMoistAirEnthalpy(
        TDryBulb,
        this.GetSatHumRatio(TDryBulb, Pressure)
      );
    };
    ...
    this.GetMoistAirVolume = function (
      ...
      if (this.isIP())
        return (
          (R_DA_IP *
            this.GetTRankineFromTFahrenheit(TDryBulb) *
            (1 + 1.607858 * BoundedHumRatio)) /
          (144 * Pressure)
        );
      else
        return (
          (R_DA_SI *
            this.GetTKelvinFromTCelsius(TDryBulb) *
            (1 + 1.607858 * BoundedHumRatio)) /
          Pressure
        );
    };
    ...
    this.GetMoistAirDensity = function (
      ...
      return (
        (1 + BoundedHumRatio) /
        this.GetMoistAirVolume(TDryBulb, BoundedHumRatio, Pressure)
      );
    };
    ...
    this.CalcPsychrometricsFromTWetBulb = function (
      ...
      var TDewPoint = this.GetTDewPointFromHumRatio(
        TDryBulb,
        HumRatio,
        Pressure
      );
      ...
      var DegreeOfSaturation = this.GetDegreeOfSaturation(
        TDryBulb,
        HumRatio,
        Pressure
      );
...
var psychrometrics = new Psychrometrics();
psychrometrics.SetUnitSystem(psychrometrics.SI);

function switchUnits(si_units) {
  if (typeof si_units !== "boolean") {
    throw new Error(
      "Is SI Units are invalid. Valid values 1 if SI Units or 0/None if not SI units."
    );
  }
  ...
  if (si_units) {
    ...
    psychrometrics.SetUnitSystem(psychrometrics.IP);
    return false;
  }
}

function checkUnits(si_units) {
  if (!(psychrometrics instanceof Psychrometrics)) {
    psychrometrics = new Psychrometrics();
  }

  if (si_units == null) {
    si_units = false; // default (IP)
  }

  const switched_units = switchUnits(si_units);
  if (switched_units) {
    return switched_units;
  }

  return si_units;
}

function checkPressure(si_units, pressure) {
  var new_pressure = pressure;
  if (si_units) {
    new_pressure = new_pressure * 1000;
  }
...
/**
 * Calculates relative humidity (%) based on T (F or C), P (psia or kPa) and state vairable. The state variable can be wet-bulb (Situation 1), dew-point (situation = 2), or humidity ratio (Situation=4).
 * @customfunction
 * @param temp Temperature
 * @param pressure Pressure
 * @param situation Situation
 * @param state_var State Variable
 * @param si_units Is SI Units
 * @returns Return dew-point temperature
 */
function humidity(temp, pressure, situation, state_var, si_units = true) {
  try {
    si_units = checkUnits(si_units);
    pressure = checkPressure(si_units, pressure);
    ...
    if (situation == 1) {
      ...
      const dry_bulb = temp;
      const wet_bulb = state_var;
      return psychrometrics.GetRelHumFromTWetBulb(dry_bulb, wet_bulb, pressure);
    } else if (situation == 2) {
      // dew_point is known
      ...
      const dry_bulb = temp;
      const dew_point = state_var;
      return psychrometrics.GetRelHumFromTDewPoint(dry_bulb, dew_point);
    } else if (situation == 3) {
      // humidity is known
      return state_var;
    } else if (situation == 4) {
      // humidity ratio is known

      // dry_bulb temperature
      // pressure
      // hum_ratio
      const dry_bulb = temp;
      const hum_ratio = state_var;
      const vap_pres = psychrometrics.GetVapPresFromHumRatio(
        hum_ratio,
        pressure
      );
      return psychrometrics.GetRelHumFromVapPres(dry_bulb, vap_pres);
    } else {
      return "ERROR: Situation must be 1-4";
    }
  } catch (err) {
...
/**
 * Calculates dew point based on T (F or C), P (psia or kPa) and state vairable. The state variable can be wet-bulb (Situation 1), relative humidity (situation = 3), or humidity ratio (Situation=4).
 * @customfunction
 * @param temp Temperature
 * @param pressure Pressure
 * @param situation Situation
 ...
 * @returns Return dew-point temperature
 */
function dewpoint(temp, pressure, situation, state_var, si_units = true) {
  try {
    si_units = checkUnits(si_units);
    pressure = checkPressure(si_units, pressure);
    ...
    if (situation == 1) {
      ...
      const dry_bulb = temp;
      const wet_bulb = state_var;
      return psychrometrics.GetTDewPointFromTWetBulb(
        dry_bulb,
        wet_bulb,
        pressure
      );
      ...
      // rel_hum
      const dry_bulb = temp;
      const rel_hum = state_var;
      return psychrometrics.GetTDewPointFromRelHum(dry_bulb, rel_hum);
    } else if (situation == 4) {
      // humidity ratio is known
      ...
      const dry_bulb = temp;
      const hum_ratio = state_var;
      return psychrometrics.GetTDewPointFromHumRatio(
        dry_bulb,
        hum_ratio,
        pressure
      );
...
/**
 * Calculates wet bulb based on T (F or C), P (psia or kPa) and state vairable. The state variable can be dew-point (situation = 2), relative humidity (situation = 3), or humidity ratio (Situation=4).
 * @customfunction
 * @param temp Temperature
 * @param pressure Pressure
 * @param situation Situation
 ...
 * @returns Return dew-point temperature
 */
function wetbulb(temp, pressure, situation, state_var, si_units = true) {
  try {
    si_units = checkUnits(si_units);
    pressure = checkPressure(si_units, pressure);
    ...
    if (situation == 1) {
      ...
      const dry_bulb = temp;
      const dew_point = state_var;
      return psychrometrics.GetTWetBulbFromTDewPoint(
        dry_bulb,
        dew_point,
        pressure
      );
      ...
      const dry_bulb = temp;
      const rel_hum = state_var;
      return psychrometrics.GetTWetBulbFromRelHum(dry_bulb, rel_hum, pressure);
    } else if (situation == 4) {
      // humidity ratio is known
      ...
      const dry_bulb = temp;
      const hum_ratio = state_var;
      return psychrometrics.GetTWetBulbFromHumRatio(
        dry_bulb,
        hum_ratio,
        pressure
      );
...
/**
 * Calculates humidity ratio based on T (F or C), P (psia or kPa) and state vairable. The state variable can be wet-bulb (Situation 1),  dew-point (situation = 2), or  relative humidity (situation = 3).
 * @customfunction
 * @param temp Temperature
 * @param pressure Pressure
 * @param situation Situation
 ...
 * @returns Return dew-point temperature
 */
function humidratio(temp, pressure, situation, state_var, si_units = true) {
  try {
    si_units = checkUnits(si_units);
    pressure = checkPressure(si_units, pressure);
    ...
    if (situation == 1) {
      ...
      const dry_bulb = temp;
      const wet_bulb = state_var;
      return psychrometrics.GetHumRatioFromTWetBulb(
        dry_bulb,
        wet_bulb,
        pressure
      );
      ...
      // dew_point temperature
      const dew_point = state_var;
      return psychrometrics.GetHumRatioFromTDewPoint(dew_point, pressure);
    } else if (situation == 3) {
      // humidity is known
      ...
      const dry_bulb = temp;
      const rel_hum = state_var;
      return psychrometrics.GetHumRatioFromRelHum(dry_bulb, rel_hum, pressure);
    } else if (situation == 4) {
      // humidity ratio is known
...
/**
 * Calculates enthalpy based on T (F or C), P (psia or kPa) and state vairable. The state variable can be wet-bulb (Situation 1),  dew-point (situation = 2), relative humidity (situation = 3), or humidity ratio (Situation=4).
 * @customfunction
 * @param temp Temperature
 * @param pressure Pressure
 * @param situation Situation
 ...
 * @returns Return dew-point temperature
 */
function enthalpy(temp, pressure, situation, state_var, si_units = true) {
  try {
    si_units = checkUnits(si_units);
    pressure = checkPressure(si_units, pressure);
    ...
    if (situation == 1) {
      ...
      const dry_bulb = temp;
      const wet_bulb = state_var;
      const hum_ratio = psychrometrics.GetHumRatioFromTWetBulb(
        dry_bulb,
        wet_bulb,
        pressure
      );
      return si_units
        ? psychrometrics.GetMoistAirEnthalpy(dry_bulb, hum_ratio) / 1000
        : psychrometrics.GetMoistAirEnthalpy(dry_bulb, hum_ratio);
    } else if (situation == 2) {
      // dew_point is known
      ...
      const dry_bulb = temp;
      const dew_point = state_var;
      const hum_ratio = psychrometrics.GetHumRatioFromTDewPoint(
        dew_point,
        pressure
      );
      return si_units
      ...
      const dry_bulb = temp;
      const rel_hum = state_var;
      const hum_ratio = psychrometrics.GetHumRatioFromRelHum(
        dry_bulb,
        rel_hum,
        pressure
      );
...
/**
 * Calculates volume based on T (F or C), P (psia or kPa) and state vairable. The state variable can be wet-bulb (Situation 1),  dew-point (situation = 2), relative humidity (situation = 3), or humidity ratio (Situation=4).
 * @customfunction
 * @param temp Temperature
 * @param pressure Pressure
 * @param situation Situation
 ...
 * @returns Return dew-point temperature
 */
function volume(temp, pressure, situation, state_var, si_units = true) {
  try {
    si_units = checkUnits(si_units);
    pressure = checkPressure(si_units, pressure);
    ...
    if (situation == 1) {
      ...
      return psychrometrics.GetMoistAirVolume(dry_bulb, hum_ratio, pressure);
    } else if (situation == 2) {
      // dew_point is known
...
/**
 * Calculates estimate of atmospheric pressure from altitude (ft or km).
 * @customfunction
 * @param altitude Altitude
 * @param units Situation
 * @returns Return dew-point temperature
 */
function altitudeAtmoP(altitude, si_units = true) {
  try {
    si_units = checkUnits(si_units);
  } catch (err) {
    return "ERROR: " + err.message;
  ...
  return si_units? psychrometrics.GetStandardAtmPressure(altitude)/1000: psychrometrics.GetStandardAtmPressure(altitude);
}

// Get Psyched Functions
CustomFunctions.associate("humidity", humidity);
CustomFunctions.associate("dewpoint", dewpoint);
CustomFunctions.associate("wetbulb", wetbulb);
CustomFunctions.associate("humidratio", humidratio);
CustomFunctions.associate("enthalpy", enthalpy);
CustomFunctions.associate("volume", volume);
CustomFunctions.associate("altitudeAtmoP", altitudeAtmoP);
